...
function rangesIntersect(
  rangeA: AbstractRange,
  rangeB: AbstractRange,
...
export function rangeToHtml(range: {
  ...
  var container = range.commonAncestorContainer.parentNode.cloneNode(
    false
  ) as Element;
...
export function createDomRangeP1<
  TBase extends Constructor<RangeP2 & RangeBase>
>(Base: TBase) {
  return class extends Base implements RangeP1Ex {
    ...
    toString() {
      ...
      if (sc === this.endContainer && isCharacterDataNode(sc)) {
        ...
        log.info(
          'toString iterator: ' +
            dom.inspectNode(iterator._first) +
            ', ' +
            dom.inspectNode(iterator._last)
        );
