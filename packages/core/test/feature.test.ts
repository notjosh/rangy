...
QUnit.test('TextRange support', function (t) {
  ...
  t.equal(
    rangy.features.implementsTextRange,
    !!browser.satisfies({ ie: '>=4' })
  );
});
...
QUnit.test('window.getSelection() support', function (t) {
  t.equal(
    rangy.features.implementsWinGetSelection,
    !browser.satisfies({ ie: '<9' })
  );
});

QUnit.test('selection has rangeCount', function (t) {
  t.equal(
    rangy.features.selectionHasRangeCount,
    !browser.satisfies({ ie: '<9' })
  );
});
...
QUnit.test('selection has anchor and focus support', function (t) {
  t.equal(
    rangy.features.selectionHasAnchorAndFocus,
    !browser.satisfies({ ie: '<9' })
  );
});
