...
import { ClassApplier } from '@notjosh/rangy-classapplier';
...
interface Converter {
  ...
  serializeSelection(
    ...
    containerNode: Node
  ): SerializedSelectionT[];

  restoreSelection(
    selection: WrappedSelection,
    savedSelection: SerializedSelectionT[],
    containerNode: Node
  ): void;
}
...
class HighlighterType {
  constructor(public type: string, public converterCreator: ConverterCreator) {}
...
export function registerHighlighterType(
  type: string,
...
var textContentConverter: Converter = {
  ...
  restoreSelection: function (selection, savedSelection, containerNode) {
    ...
    for (
      var i = 0, len = savedSelection.length, range, rangeInfo, characterRange;
      i < len;
      ++i
    ) {
    ...
    ranges.forEach(function (range) {
      ...
      highlights.forEach(function (highlight) {
        if (
          ...
          !intersectingHighlights.includes(highlight)
        ) {
