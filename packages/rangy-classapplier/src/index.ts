...
function canTextBeStyled(textNode) {
  ...
  return (
    parent &&
    parent.nodeType == 1 &&
    !/^(textarea|style|script|select|iframe)$/i.test(parent.nodeName)
  );
}
...
function movePosition(position, oldParent, oldIndex, newParent, newIndex) {
  ...
  if (
    posNode == oldParent &&
    (posOffset == oldIndex || posOffset == oldIndex + 1)
  ) {
    newNode = newParent;
...
function movePreservingPositions(
  node,
  newParent,
  newIndex,
  positionsToPreserve
...
function replaceWithOwnChildrenPreservingPositions(
  element,
  positionsToPreserve
) {
  return moveChildrenPreservingPositions(
    element,
    element.parentNode,
    dom.getNodeIndex(element),
    true,
    positionsToPreserve
  );
}
...
function isEditable(node) {
  return (
    (isEditableElement(node) ||
      (node.nodeType != 1 && isEditableElement(node.parentNode))) &&
    !isEditingHost(node)
  );
}
...
function isUnrenderedWhiteSpaceNode(node) {
  ...
  return (
    isNonInlineElement(node.previousSibling) ||
    isNonInlineElement(node.nextSibling)
  );
}

function getRangeBoundaries(ranges: AbstractRange[]) {
  var positions = [],
    i,
    range;
...
function splitNodeAt(
  ...
  if (isSplitPoint(descendantNode, descendantOffset)) {
    ...
    movePreservingPositions(
      newNode,
      parentNode,
      dom.getNodeIndex(descendantNode) + 1,
      positionsToPreserve
    );
...
function areElementsMergeable(el1, el2) {
  return (
    el1.namespaceURI == el2.namespaceURI &&
...
class Merge {
  isElementMerge: boolean;
  textNodes: CharacterData[];
...
export class ClassApplier {
  ...
  tagNames: string[];
  cssClass: string;
  applyToAnyTagName: boolean;
  attrExceptions: string[];
  ...
  constructor(
    ...
    applier.elementProperties = applier.copyPropertiesToElement(
      elementPropertiesFromOptions,
      el,
      true
    );
  ...
  elementTagName: keyof HTMLElementTagNameMap = defaultTagName;
  elementProperties: {
    className?: string;
  };
  elementAttributes: object;
  ...
  onElementCreate?: (Element, ClassApplier) => void;

  copyPropertiesToElement(props, el, createCopy) {
    var s,
      elStyle,
      elProps = {},
      elPropsStyle,
      ...
      if (props.hasOwnProperty(p)) {
        ...
        // Special case for style
          ...
          if (createCopy) {
            ...
            attrName = attrNamesForProperties.hasOwnProperty(p)
              ? attrNamesForProperties[p]
              : p;
  ...
  isIgnorableWhiteSpaceNode(node) {
    return (
      ...
      node &&
      node.nodeType == 3 &&
      isUnrenderedWhiteSpaceNode(node)
    );
  }
  ...
  postApply(textNodes, range, positionsToPreserve, isUndo) {
    ...
    if (merges.length) {
      ...
      log.info(
        rangeStartNode.nodeValue,
        rangeStartOffset,
        rangeEndNode.nodeValue,
        rangeEndOffset
      );
  ...
  createContainer(parentNode) {
    ...
    var namespace;
    var el =
      createElementNSSupported &&
      !dom.isHtmlNamespace(parentNode) &&
  ...
  applyToTextNode(textNode, positionsToPreserve) {
    log.group(
      "Apply class '" + this.className + "'. textNode: " + textNode.data
    );
    log.info(
      "Apply class  '" + this.className + "'. textNode: " + textNode.data
    );
  ...
  isEmptyContainer(el) {
    ...
    return (
      ...
      this.isRemovable(el) &&
      (childNodeCount == 0 ||
        (childNodeCount == 1 && this.isEmptyContainer(el.firstChild)))
    );
  }

  removeEmptyContainers(range) {
    var applier = this;
  ...
  undoToTextNode(textNode, range, ancestorWithClass, positionsToPreserve) {
    ...
    if (!range.containsNode(ancestorWithClass)) {
      ...
      if (
        ancestorRange.isPointInRange(range.startContainer, range.startOffset)
      ) {
    ...
    log.info(
      ...
      "'" + ancestorWithClass.parentNode.innerHTML + "'"
    );
  ...
  splitAncestorWithClass(container, offset, positionsToPreserve) {
  ...
  applyToRange(range, rangesToPreserve?) {
    var applier = this;
    ...
    if (textNodes.length) {
      textNodes.forEach(function (textNode) {
        ...
        if (
          !applier.isIgnorableWhiteSpaceNode(textNode) &&
          !applier.getSelfOrAncestorWithClass(textNode) &&
  ...
  undoToRange(range, rangesToPreserve?) {
    var applier = this;
    // Create an array of range boundaries to preserve
    rangesToPreserve = rangesToPreserve || [];
    var positionsToPreserve = getRangeBoundaries(rangesToPreserve);
    ...
    if (textNodes.length) {
      applier.splitAncestorWithClass(
        range.endContainer,
        range.endOffset,
        positionsToPreserve
      );
      ...
      log.info(
        "Undo set range to '" + textNodes[0].data + "', '" + textNode.data + "'"
      );
    ...
    if (range.collapsed || range.toString() == '') {
      ...
      if (textNodes.length)
        for (var i = 0, textNode; (textNode = textNodes[i++]); ) {
          if (
            !this.isIgnorableWhiteSpaceNode(textNode) &&
            rangeSelectsAnyText(range, textNode) &&
            this.isModifiable(textNode) &&
            !this.getSelfOrAncestorWithClass(textNode)
          ) {
            return false;
          }
        }
  ...
  getElementsWithClassIntersectingRange(range) {
    var elements = [];
    var applier = this;
