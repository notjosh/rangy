...
export class RangeIterator {
  ...
  constructor(
    public range: IterableRange,
    public clonePartiallySelectedTextNodes
  ) {
    log.info(
      'New RangeIterator ',
    ...
    if (!range.collapsed) {
      ...
      log.info(
        'RangeIterator first and last',
        dom.inspectNode(this._first),
        dom.inspectNode(this._last)
      );
    }
  }
    ...
    if (current) {
      ...
      if (
        ...
        if (current === this.ec) {
          ...
          (current as CharacterData).deleteData(
            this.eo,
            (current as CharacterData).length - this.eo
          );
        }
  ...
  remove() {
    ...
    if (
      ...
      if (current.parentNode) {
        ...
        log.warn(
          'Node to be removed has no parent node. Is this the child of an attribute node in Firefox 2?'
        );
      }
    }
  }
...
export function cloneSubtree(iterator: RangeIterator) {
  ...
  for (
    ...
    log.debug(
      'cloneSubtree got node ' +
        dom.inspectNode(node) +
        ' from iterator. partiallySelected: ' +
        partiallySelected
    );
