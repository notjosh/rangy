...
import { RangyRangeEx, WrappedSelection } from '@notjosh/rangy-core';
...
function testSelectionAndRangeCreators(
  wins,
  winName,
  selectionCreator: SelectionCreator,
  selectionCreatorName,
  rangeCreator,
  rangeCreatorName
) {
...
*/
    ...
    QUnit.testEx(
      'Adding mutiple ranges where only one is supported',
      function (t) {
        rangy.config.checkSelectionRanges = false;
        var sel = selectionCreator(win);
        sel.removeAllRanges();
  ...
  if (rangy.features.collapsedNonEditableSelectionsSupported) {
    QUnit.testEx(
      'Collapse same document test (non-editable)',
      function (t) {
        var sel = selectionCreator(win);
        sel.removeAllRanges();
        var range = rangeCreator(doc);
    ...
    QUnit.skipEx(
      'Collapse other document test (non-editable)',
      function (t) {
        var sel = selectionCreator(win);
        sel.removeAllRanges();
        var range = rangeCreator(doc);
...
*/
    ...
    QUnit.testEx(
      'collapseToStart test (non-editable)',
      function (t) {
        var sel = selectionCreator(win);
        sel.removeAllRanges();
        var range = rangeCreator(doc);
    ...
    QUnit.testEx(
      'Test collapsed selections cannot exist in non-editable elements',
      function (t) {
        var sel = selectionCreator(win);
        sel.removeAllRanges();
        var range = rangeCreator(doc);
  ...
  QUnit.testEx(
    'Collapse same document test (editable)',
    function (t) {
  ...
  QUnit.skipEx(
    'Collapse other document test (editable)',
    function (t) {
      nodes.div.contentEditable = 'true';
      var sel = selectionCreator(win);
      sel.removeAllRanges();
...
*/
  ...
  QUnit.testEx(
    'selectAllChildren same document test',
    function (t) {
      var sel = selectionCreator(win);
      sel.removeAllRanges();
      var range = rangeCreator(doc);
  ...
  QUnit.testEx(
    'HTML5 toString script contents test',
    function (t) {
      var div = doc.createElement('div');
      div.innerHTML =
        'one<script type="text/javascript">var x = 1;</script>two';
      doc.body.appendChild(div);
  ...
  QUnit.testEx(
    'HTML5 toString display:none contents test',
    function (t) {
      var div = doc.createElement('div');
  ...
  function isNativeSel(
    selectionCreator,
    method: string
  ): selectionCreator is (win) => Selection {
    return testSelection[method];
  }
  function isWrappedSel(
    selectionCreator,
    ...methods: string[]
  ): selectionCreator is (win) => WrappedSelection {
  ...
  function testRefresh(
    name: string,
    testRangeCreator: (t: Assert) => RangyRangeEx
  ) {
    if (isWrappedSel(selectionCreator, 'refresh')) {
      QUnit.test('Refresh test: ' + name, function (t) {
        ...
        t.ok(
          DomRange.rangesEqual(range, selRange),
          'Ranges not equal. Original: ' +
            DomRange.inspect(range) +
            ', refreshed selection range: ' +
            DomRange.inspect(selRange)
        );
      });
    }
  }
...
testSelectionAndRangeCreators(
  [window],
  'main',
  createRangySelection,
  'Rangy Selection',
  createRangyRange,
  'Rangy Range'
);
...
window.addEventListener('load', function () {
  ...
  doc.open();
  doc.write(
    '<html><head><title>Rangy Selection Test</title></head><body>Content</body></html>'
  );
  doc.close();
