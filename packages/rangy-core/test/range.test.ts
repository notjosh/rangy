...
function testRangeCreator(
  ...
  const hooks: Hooks = {
    beforeEach: function () {
      ...
      var boldAndItalicText = i.appendChild(
        doc.createTextNode('bold and italic')
      );
  ...
  function isDomRange(
    rangeCreator,
    testMethod: string
  ): rangeCreator is (doc: Document) => DomRange {
  ...
  QUnit.test('compareBoundaryPoints 1', function (t) {
    ...
    t.strictEqual(
      range1.compareBoundaryPoints(range1.START_TO_START, range2),
      1
    );
  ...
  if (testRange.intersectsNode) {
    ...
    QUnit.test(
      'intersectsNode test boundary at end of text node',
      function (t) {
        var range = rangeCreator(doc);
        range.setStart(nodes.plainText, nodes.plainText.length);
        range.setEnd(nodes.boldText, 1);
    ...
    QUnit.test(
      'intersectsNode test touching is not intersecting',
      function (t) {
        var range = rangeCreator(doc);
    ...
    if (testRange.intersectsNode.length == 2) {
      QUnit.test(
        'intersectsNode touching is intersecting at start',
        function (t) {
          var range = rangeCreator(doc);
          range.setStart(nodes.plainText, 0);
      ...
      QUnit.test(
        'intersectsNode touching is intersecting at end',
        function (t) {
          var range = rangeCreator(doc);
          range.setStartAfter(nodes.plainText);
          range.setEnd(nodes.boldText, 1);
  ...
  if (isDomRange(rangeCreator, 'splitBoundaries')) {
    ...
    QUnit.test(
      'Concatenate getNodes([3]) after splitBoundaries same as toString - simple',
      function (t) {
        var range = rangeCreator(doc);
    ...
    QUnit.test(
      'Concatenate getNodes([3]) after splitBoundaries same as toString - end at position 0 in text node (issue 190)',
      function (t) {
        var range = rangeCreator(doc);
    ...
    QUnit.test(
      'Concatenate getNodes([3]) after splitBoundaries same as toString - start position at end of text node (issue 190)',
      function (t) {
        var range = rangeCreator(doc);
    ...
    QUnit.test(
      'Concatenate getNodes([3]) after splitBoundaries same as toString - start position at end of text node and end at position 0 in text node (issue 190)',
      function (t) {
        var range = rangeCreator(doc);
  ...
  if (isDomRange(rangeCreator, 'normalizeBoundaries')) {
    ...
    QUnit.test(
      'normalizeBoundaries collapsed at start of element first child',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
    ...
    QUnit.test(
      'normalizeBoundaries collapsed at start of element text node',
      function (t) {
        var range = rangeCreator(doc);
    ...
    QUnit.test(
      'normalizeBoundaries collapsed at end of element last child',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries collapsed at end of element text node',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when collapsed range is at end of text node that is immediately followed by another',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when collapsed range is between two text nodes',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when the end of an uncollapsed range is at end of text node that is immediately followed by another',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when the end of an uncollapsed range is between two text nodes',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when the start of an uncollapsed range is between two text nodes',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when the start of an uncollapsed range is at start of text node that is immediately preceded by another',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when the start of an uncollapsed range is at end of text node that is immediately followed by another',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
    ...
    QUnit.test(
      'normalizeBoundaries when the end of an uncollapsed range is at start of text node that is immediately preceded by another',
      function (t) {
        var range = rangeCreator(doc);
        var b = nodes.b;
        b.innerHTML = '';
  ...
  if (testRange.createContextualFragment) {
    QUnit.test(
      'createContextualFragment create b fragment in xmp',
      function (t) {
        var range = rangeCreator(doc);
    ...
    QUnit.test(
      'createContextualFragment create b fragment in div',
      function (t) {
        var range = rangeCreator(doc);
...
function testAcid3(rangeCreator, rangeCreatorName) {
  ...
  QUnit.test('Acid3 test 7: basic ranges initial position tests', function (t) {
    ...
    t.equal(
      r.cloneContents().childNodes.length,
      0,
      'nothing cloned was more than nothing'
    );
    t.equal(
      r.cloneRange().toString(),
      '',
      'nothing cloned stringifed to more than nothing'
    );
    r.collapse(true); // no effect
    t.equal(
      r.compareBoundaryPoints(r.START_TO_END, r.cloneRange()),
      0,
      "starting boundary point of range wasn't the same as the end boundary point of the clone range"
    );
    r.deleteContents(); // no effect
    t.equal(
      r.extractContents().childNodes.length,
      0,
      'nothing removed was more than nothing'
    );
    ...
    try {
      ...
      t.equal(
        r.startOffset,
        0,
        "range with inserted comment has start offset that isn't zero"
      );
      t.equal(
        r.endContainer,
        document,
      ...
      t.equal(
        r.endOffset,
        1,
        "range with inserted comment has end offset that isn't after the comment"
      );
    } finally {
  ...
  QUnit.test('Acid3 test 8: moving boundary points', function (t) {
    ...
    try {
      ...
      /*
        ...
        e.code != e.INVALID_NODE_TYPE_ERR
      )
    ...
    t.equal(
      r.startOffset,
      0,
      'startOffset is wrong after selectNodeContents()'
    );
    t.equal(
      r.endContainer,
      root,
  ...
  QUnit.test('Acid3 test 9: extractContents() in a Document', function (t) {
    ...
    t.equal(
      r.toString(),
      'Hello Wonderful KittyHow are you?',
    ...
    t.equal(
      r.toString(),
      'ful Kitty',
      'toString() on range crossing text nodes gave wrong output'
    );
    ...
    t.ok(
      ...
      'expected two children in the result, got ' + f.childNodes.length
    );
    ...
    t.equal(
      f.childNodes[0].childNodes[0].childNodes[0].data,
      'ful',
      'failure 9'
    );
  ...
  QUnit.test(
    'Acid3 test 12: Ranges under mutations: insertion into text nodes',
    function (t) {
      var doc = getTestDocument();
      var p = doc.createElement('p');
      ...
      t.equal(
        p.childNodes.length,
        3,
        'insertion of node made wrong number of child nodes'
      );
      t.equal(p.childNodes[0], t1, 'unexpected first text node');
      t.equal(
        (p.childNodes[0] as CharacterData).data,
        '12',
        'unexpected first text node contents'
      );
      t.equal(p.childNodes[1], t2, 'unexpected second text node');
      t.equal(
        (p.childNodes[1] as CharacterData).data,
        'ABCDE',
        'unexpected second text node'
      );
      t.equal(
        (p.childNodes[2] as CharacterData).data,
        '345',
        'unexpected third text node contents'
      );
...
testRangeCreator(
  [document],
  'main',
  createWrappedNativeDomRange,
  'Wrapped native Range'
);
...
window.addEventListener('load', function () {
  ...
  doc.open();
  doc.write(
    '<html><head><title>Rangy Test</title></head><body>Content</body></html>'
  );
  doc.close();
  iframeDoc[0] = doc;
});
